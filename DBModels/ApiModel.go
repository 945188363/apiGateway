...
func (p *Api) GetApi() error {
	if err := DB.DBConn().Find(&p,"api_name = ?",p.ApiName).Error; err != nil{
		log.Fatal(err)
		return err
	}
	return nil
...
func (p *Api) SaveApi()  bool{
	// 已存在更新，否则创建
	exist := Api{}
	DB.DBConn().Find(&exist,"api_name=?",p.ApiName)
	if exist.Id != 0 {
		updateApi := Api{
			ApiName:          p.ApiName,
			ApiUrl:           p.ApiUrl,
...
func (p *Api) DeleteApi() bool {
	if err := DB.DBConn().Where("api_name = ?",p.ApiName).Delete(&p).Error;err != nil{
		log.Fatal(err)
		return false
	}
	return true
